/** @file list.c
 *  @brief Implementation of a linked list.
 *
 * Based on the implementation approach described in "The Practice
 * of Programming" by Kernighan and Pike (Addison-Wesley, 1999).
 *
 */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "emalloc.h"
#include "list.h"

/**
 * Function:  new_node
 * -------------------
 * @brief  Allows to dynamically allocate memory for a new node to be added to the linked list.
 *
 * This function should confirm that the argument being passed is not NULL (i.e., using the assert library). Then,
 * It dynamically allocates memory for the new node using emalloc(), and assign values to attributes associated with the node (i.e., val and next).
 *
 * @param val The value to be associated with the node.
 *
 * @return node_t* A pointer to the node created.
 *
 */
node_t *new_node(char *name, char *airline_icao, char *to_name, char *to_city, char *to_country, char *airport_icao)
{
    assert(name != NULL);

    node_t *temp = (node_t *)emalloc(sizeof(node_t));

    temp->airline_name = strdup(name);
    temp->airline_icao_unique_code = strdup(airline_icao);
    temp->to_airport_name = strdup(to_name);
    temp->to_airport_city = strdup(to_city);
    temp->to_airport_country = strdup(to_country);
    temp->to_airport_icao_unique_code = strdup(airport_icao);
    temp->statistic = 1;
    temp->next = NULL;

    return temp;
}

/**
 * Function:  add_front
 * --------------------
 * @brief  Allows to add a node at the front of the list.
 *
 * @param list The list where the node will be added (i.e., a pointer to the first element in the list).
 * @param new The node to be added to the list.
 *
 * @return node_t* A pointer to the new head of the list.
 *
 */
node_t *add_front(node_t *list, node_t *new)
{
    new->next = list;
    return new;
}

/**
 * Function:  add_end
 * ------------------
 * @brief  Allows to add a node at the end of the list.
 *
 * @param list The list where the node will be added (i.e., a pointer to the first element in the list).
 * @param new The node to be added to the list.
 *
 * @return node_t* A pointer to the head of the list.
 *
 */
node_t *add_end(node_t *list, node_t *new)
{
    node_t *curr;

    if (list == NULL)
    {
        new->next = NULL;
        return new;
    }

    for (curr = list; curr->next != NULL; curr = curr->next)
        ;
    curr->next = new;
    new->next = NULL;
    return list;
}

/**
 * Function:  add_inorder
 * ----------------------
 * @brief  Allows to add a new node to the list respecting an order.
 *
 * @param list The list where the node will be added (i.e., a pointer to the first element in the list).
 * @param new The node to be added to the list.
 *
 * @return node_t* A pointer to the node created.
 *
 */
node_t *add_inorder(node_t *list, node_t *new, int question){
    node_t *prev = NULL;
    node_t *curr = NULL;

    if (list == NULL){
        return new;
    }

    for (curr = list; curr != NULL; curr = curr->next){
        if (question == 1){
        // Nodes will be placed from largest to smallest statistic, in alphabetical order
            if (new->statistic == curr->statistic && strcmp(new->airline_name, curr->airline_name) > 0){
                prev = curr;
            }else if (new->statistic < curr->statistic){
                prev = curr;
            }else{
                break;
            }
        }
        if (question == 2){
        // Nodes will be places from smallest to largest statistic. in alphabetical order
            if (new->statistic == curr->statistic && strcmp(new->to_airport_country, curr->to_airport_country) > 0){
                prev = curr;
            }else if (new->statistic > curr->statistic){
                prev = curr;
            }else{
                break;
            }
        }
        if(question == 3){
            // Nodes will be placed from largest to smallest statistic, in alphabetical order
            if(new->statistic == curr->statistic && strcmp(new->to_airport_name, curr->to_airport_name) > 0){
                prev = curr;
            }else if (new->statistic < curr->statistic){
                prev = curr;
            }else{
                break;
            }
        }
    }

    new->next = curr;

    if (prev == NULL){
        return (new);
    }
    else{
        prev->next = new;
        return list;
    }
}

/**
 * Function:  peek_front
 * ---------------------
 * @brief  Allows to get the head node of the list.
 *
 * @param list The list to get the node from.
 *
 * @return node_t* A pointer to the head of the list.
 *
 */
node_t *peek_front(node_t *list)
{
    return list;
}

/**
 * Function:  remove_front
 * -----------------------
 * @brief  Allows removing the head node of the list.
 *
 * @param list The list to remove the node from.
 *
 * @return node_t* A pointer to the head of the list.
 *
 */
node_t *remove_front(node_t *list)
{
    if (list == NULL)
    {
        return NULL;
    }

    return list->next;
}

/**
 * Function: apply
 * --------------
 * @brief  Allows to apply a function to the list.
 *
 * @param list The list (i.e., pointer to head node) where the function will be applied.
 * @param fn The pointer of the function to be applied.
 * @param arg The arguments to be applied.
 *
 */
void apply(node_t *list,
           void (*fn)(node_t *list, void *),
           void *arg)
{
    for (; list != NULL; list = list->next)
    {
        (*fn)(list, arg);
    }
}

/**
 * Funcion: searchList
 * -------------------
 * @brief Checks a list to see if it contains a given node, parameters based on question number
 * 
 * @param list The list (pointer to the head node) that the function is applied
 * @param new A newly created node, a reference to check if the node's value is already in the list
 * @param question The question given from the command line, helps determines the value being referenced from 'new'
*/
node_t *searchList(node_t *list, node_t *new, int question){
    node_t *prev = NULL;
    node_t *curr = NULL;
    int inList = 0;

    if (list == NULL){
        return new;
    }
    for (curr = list; curr != NULL; curr = curr->next){
        if ((question == 1 && strcmp(new->airline_name, curr->airline_name) == 0) || (question == 2 && strcmp(new->to_airport_country, curr->to_airport_country) == 0) || (question == 3 && strcmp(new->to_airport_name, curr->to_airport_name) == 0)){
            curr->statistic = curr->statistic + 1;
                inList++;
                if (prev == NULL){
                    list = curr->next;          //updating the head pointer of the list
                    curr->next = NULL;          //taking out the current head pointer
                    list = add_inorder(list, curr, question);
                 } else {
                    prev->next = curr->next;
                    curr->next = NULL;
                    list = add_inorder(list, curr, question);
                }
        }
        prev = curr;
    }
    if(inList == 0){
        list = add_inorder(list, new, question);
    }
    return list;
}
